...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Colman Bashore"
date: "`r Sys.Date()`"
...
packages <- c(
  ...
  "geepack", "knitr", "kableExtra", "foreign", "broom", "dotwhisker", "dotenv", "rmapshaper"
)
...
missing_data <- filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry()
missing_data <- t(missing_data)


missing_data %>% kable(caption = "Missing Data") %>% kable_styling() %>%
...
Use an updated cartography method to make the map more accessible
```{r map-covid-rates-alt, message = FALSE}
tm_covid_rates <- tm_shape(acs_covid) +
  tm_polygons("covid_rate",
    title = "COVID-19 Cases per 100,000 people\n(22 January 2020 to 1 August 2020)",
    style = "quantile",
  ...
  tm_shape(state, simplify = .01) +
    tm_borders("white", lwd = .4) +
  tm_layout(
    legend.position = c("left", "bottom"),
    legend.title.size = 0.8,
    legend.text.size = 0.5
...
# Updated mapping parameters to improve accessibility and communication
...
## Interpretation of Cartographic Adjustments
A few adjustments have been made to the mapping code chunks with the goal of improving visibility and reducing confusion. These adjustments focus mostly on borders and using the tmap package. For the covid cases map, the country borders have been made transparent as there were a lot of distracting artefacts along the more complicated borders. Additionally, the rmapshaper package has been added to use the tmap parameter "simplify." The linework for the state borders has been simplified by a factor of 50, changed from grey to white, and been adjusted from a .5 line width to .4. All of these adjustments serve to reduce artefacts and complexities that are uneccesary. Particularly at county borders that follow rivers and are thus quite complicated, as well as state borders that surround small islands or complicated coastlines. The color of borders changes depending on the map as some maps will have more contrast with a white border and some will benefit from sticking with the grey border. I believe that the adjustments are generally succesful in improving the cartographic quality of the maps using fairly simple adjustments. 
...
The final map of Race Model weights demonstrates the significance to the model of several counties at the center of large clusters.
There are several counties within Florida, Mississippi, Arkansas, and Tennessee that are weighted very heavily in the model but have low populations and don't necessarily make sense as "centers" of clusters but they might have been chosen because the were close to the centroid points of clusters. Comparing this map to the map of residuals reveals some interesting patterns. Many of the counties that are weighted the heaviest also have the highest error. This may indicate that some of the method of weighting may be somewhat flawed especially due to the large amount of COVID prevalence in the south. Overall, there was much more error in the south which may connect to an assumption of spatial heterogeneity and not taking into account the uneven spatial distribution of COVID at the time of the original study. 
...
# Conclusions
This reproduction attempted to reproduce Chakraborty's study using fully open source software and the goal of verifying results and methods.While our reproduction varied in the methods and computational environments that were used and we had some slightly different results, overall we were able to follow the methodology of Chakraborty. A primary point of disagreement between the two studies was the conceptualization around clustering. This was primarily an issue in the way that Chakraborty's methodology erased local epicenters that were part of a massive cluster such as the American southeast. Therefore, the center of this cluster is a relatively insignificant county. Our methodology attempted to highlight relative risk true epicenters such as Atlanta or Miami. While our results still generally suggest similar patterns of multiple jeopardy for PwD's the reproducibility and methodology of the original study can be questioned. At many points we were forced to deviate from the original methodology. Often this was due to unclear descriptions of particular tools and steps taken. This statement is not intended to discount the scientific impact of this study, but rather emphasize the difficulty and significance of providing clear and unambigous methodological descriptions in science.
